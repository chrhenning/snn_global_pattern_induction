...
"""
@title           :equations/Diehl_Cook_SNN_Three_Factor.py
@author          :ch
...
This module is an implementation of the neural dynamics and three-factor
learning rule presented in:
...
Note, we do not claim that we managed to identically transfer their equations
into this module.

Note, the initializations are chosen mostly by us, as we did not attempted to
adopt their implementation from:
    http://www.ini.uzh.ch/admin/extras/doc_get.php?id=52538

Note, that the hyperparameters in this module are tuned for a specific network,
receiving MNIST samples.
"""
...
ne_namespace = {
    ...
    'tc_theta': 1e7 * b2.ms,     # Time constant for adapive threshold var
...
ee_on_pre = '''
    pre = 1.
    w = clip(w - (nu_ee_pr * post1) * plastic, 0, wmax_ee)
    ge += w
'''
# Update on postsynaptic spike considers last post- and last presynaptic spike.
ee_on_post = '''
    post2before = post2
    w = clip(w + (nu_ee_po * pre * post2before) * plastic, 0, wmax_ee)
    post1 = 1.
    post2 = 1.
'''
ee_delay = None
ee_namespace = {
    ...
    'nu_ee_pr':  0.0001,     # Learning rate on pre
    'nu_ee_po': 0.01,        # Learning rate on post
    'wmax_ee': 1.0           # Maximum excitatory weight
}
ee_initialization = {
    'w': lambda size: winit.uniform(size, wmin=0, wmax=.3),
    'delay': lambda size: winit.uniform(size, wmin=0, wmax=10) * b2.ms
}
